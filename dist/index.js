...
var QueueList = async (options) => {
  ...
  try {
    const canvas = (0, import_canvas7.createCanvas)(690, 400);
    const ctx = canvas.getContext("2d");
    if (options.backgroundImage) {
      try {
...
var MostPlayed = async (options) => {
  ...
  try {
    const canvas = (0, import_canvas8.createCanvas)(690, 194);
    const ctx = canvas.getContext("2d");
    if (options.backgroundImage) {
      try {
        const darknessSvg = generateSvg(`<svg width="690" height="194" viewBox="0 0 690 194" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        const image = await (0, import_cropify7.cropImage)({
          //@ts-ignore
          imagePath: options.backgroundImage,
          width: 690,
          height: 194,
        ...
        ctx.drawImage(await (0, import_canvas8.loadImage)(image), 0, 0);
        ctx.drawImage(await (0, import_canvas8.loadImage)(darknessSvg), 0, 0);
      } catch (_error) {
        const backgroundSvg = generateSvg(`<svg width="690" height="194" viewBox="0 0 690 194" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect width="690" height="194" rx="35" fill="${options.backgroundColor}"/>
                </svg>`);
...
var AddedToQueue = async (options) => {
  ...
  try {
    const canvas = (0, import_canvas9.createCanvas)(500, 150);
    const ctx = canvas.getContext("2d");
    if (options.backgroundImage) {
      try {
        ...
        const image = await (0, import_cropify8.cropImage)({
          //@ts-ignore
          imagePath: options.backgroundImage,
          width: 500,
          height: 150,
        ...
        ctx.drawImage(await (0, import_canvas9.loadImage)(image), 0, 0);
        ctx.drawImage(await (0, import_canvas9.loadImage)(darknessSvg), 0, 0);
      } catch (_error) {
        ...
        const backgroundColor = await (0, import_canvas9.loadImage)(backgroundSvg);
        ctx.drawImage(backgroundColor, 0, 0);
      }
    } else {
      const backgroundSvg = generateSvg(`<svg width="500" height="150" viewBox="0 0 500 150" fill="none" xmlns="http://www.w3.org/2000/svg">
